...
def count_inter_and_uni(pred,gt):
    inter = (pred*gt)
    uni = pred+gt-inter 
    return inter.sum(), uni.sum() 



def test_single_volume(im, lab, net, classes, patch_size=[256, 256], test_save_path=None,  case=None, z_spacing=1, device= None):
    image, label = im.squeeze(0).cpu().detach().numpy(), lab.squeeze(0).cpu().detach().numpy()
    #net = net.to("cpu")
    ...
    #metric_list = []
    # intersection_of_all_classes = [] 
    # union_of_all_classes = []
    #dice_score = DiceMetric(torch.from_numpy(prediction), torch.from_numpy(label),num_classes=3,average = 'micro', ignore_index=0)
    # for i in range(1, classes):
    #     pred_class = (prediction==i).astype(int)
    #     label_class = (label==i).astype(int)
    #     metric_list.append(calculate_metric_percase(pred_class, label_class))
        # intersec, union = count_inter_and_uni(pred_class, label_class)
        # intersection_of_all_classes.append(intersec)
        # union_of_all_classes.append(union)
    ...
    return dice, torch.from_numpy(prediction).squeeze().cpu(), torch.from_numpy(label).squeeze().cpu()